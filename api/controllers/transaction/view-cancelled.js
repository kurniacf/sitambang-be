    ...
    fn: async function (inputs, exits) {
        try {
            ...
            if (data.role === 'buyer') {
                ...
                transactionData = await Transaction.find({ idBuyer: data.id, statusPayment: 'cancelled' });
            } else if (data.role === 'employee' || data.role === 'admin') {
                if (idBuyer) {
                    transactionData = await Transaction.findOne({idBuyer: idBuyer, statusPayment: 'cancelled'});
                } else {
                    transactionData = await Transaction.find({statusPayment: 'cancelled'});
                }
            }
